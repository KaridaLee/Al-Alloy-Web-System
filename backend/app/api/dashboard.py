    ...
    with engine.begin() as conn:
        ...
        for s in sheets:
            ...
            if "炉号" in safe_cols:
                ...
                row_count = conn.execute(text(f'SELECT COUNT(1) AS cnt FROM "{table_name}"')).scalar() or 0
            ...
            if "牌号" in safe_cols and "炉号" in safe_cols:
                rows = conn.execute(text(f'''
                    ...
                    GROUP BY "牌号"
                ''')).mappings().all()
                for r in rows: brand_stats[r["name"]] = brand_stats.get(r["name"], 0) + r["cnt"]
...
def get_brand_trends(brand: str = Query(..., description="牌号名称")):
    ...
    with engine.begin() as conn:
        ...
        for m in metas:
            ...
            if "牌号" not in cols or "炉号" not in cols or "批次号" not in cols: continue
            ...
            sql = f'''
                ...
                WHERE "牌号" = :brand AND "批次号" LIKE :batch_filter
                ORDER BY "{time_col}" DESC LIMIT :limit
            '''
            rows = conn.execute(text(sql), {"brand": brand, "batch_filter": "%DZ%", "limit": limit}).mappings().all()
            for r in rows: all_data.append(dict(r))
    ...
    for el in ELEMENTS_ORDER:
        ...
        trends[el] = vals
        if has_value: valid_elements.append(el)
